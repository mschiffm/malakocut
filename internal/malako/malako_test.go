...
func TestProcessPacketAndBuffer(t *testing.T) {
	...
	m, _ := NewMalakocut(Config{BufferPath: tmpDir, HTTPClient: &http.Client{}, LogType: "TEST", IngestionURL: "http://localhost", SendGridKey: ""})
	defer m.Close()
...
func TestUploadRetryLogic(t *testing.T) {
	...
	m, _ := NewMalakocut(Config{BufferPath: tmpDir, HTTPClient: &http.Client{Transport: tripper}, LogType: "TEST", IngestionURL: "http://localhost", SendGridKey: ""})
	defer m.Close()
